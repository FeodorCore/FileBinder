...
class FileBinderFilter:
    def __init__(
        ...
        self.ignore_file: bool = ignore_file
        self.hidden_files: bool = hidden_files
        self.name_file_ignore: Path = Path(name_file_ignore)
        self.name_file_binder: Path = Path(name_file_binder)
    ...
    def filters(self) -> list[Path]:
        ...
        for file in self.current_dir.rglob("*"):
            file: Path = file.relative_to(self.current_dir)
            if file.is_file():
                ...
                if self.ignore_patterns is not None and any(
                    ...
                    continue
                if self.ignore_patterns is not None and any(
                    str(file).startswith(i + "/")
                    for i in self.ignore_patterns
                ):
                    continue
                if (
                    file == self.name_file_binder
                    or file == self.name_file_ignore
                ):
                    continue
                self.list_return_files.append(file)
